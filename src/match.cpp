...
Match::Match(const YAML::Node& Yaml, MatchTable* MatchTable, ITournament* Tournament) : m_Tournament(Tournament)
{
	...
	if (Yaml["is_best_of_three"])
		m_IsBestOfThree = Yaml["is_best_of_three"].as<bool>();
...
void Match::operator >>(YAML::Emitter& Yaml) const
{
	...
	if (m_IsBestOfThree)
		Yaml << YAML::Key << "is_best_of_three" << YAML::Value << m_IsBestOfThree;
	if (IsAutoGenerated())
...
{
	m_White.m_DependentMatch = Reference1;
	m_Blue.m_DependentMatch  = Reference2;
	m_IsBestOfThree = true;
}
