...
TEST(Pool, Count4)
{
	...
	EXPECT_EQ(semi2->GetDependencyTypeOf(Fighter::White), DependencyType::TakeRank2);
	EXPECT_EQ(*semi2->GetDependentMatchTableOf(Fighter::White), *w.GetPool(0));
	EXPECT_EQ(semi2->GetDependencyTypeOf(Fighter::Blue),  DependencyType::TakeRank1);
	EXPECT_EQ(*semi2->GetDependentMatchTableOf(Fighter::Blue),  *w.GetPool(1));
